...
const startQuiz = () => {
  const kategoriId = materi.value.kategori?.id
  const topikId = materi.value.topik?.id

  if (kategoriId && topikId) {
    router.push({
      path: `/soal/${kategoriId}/${topikId}`
    })
  } else {
    console.error('Kategori atau Topik ID tidak ditemukan.')
  }
}
